...
namespace PornHub
{
        ...
        public static void Initialize(string name, string aid, string secret, string version)
        {
            ...
            using (WebClient wc = new WebClient())
            {
                ...
                try
                {
                    ...
                    if (Security2.MaliciousCheck(response[1]))
                    {
                    ...
                    {
                        MessageBox.Show("Security2 error has been triggered!", Name, MessageBoxButton.OK, MessageBoxImage.Error);
                        Process.GetCurrentProcess().Kill();
                    }
                    switch (response[2])
                    {
                        case "success":
                            ...
                            if (ApplicationSettings.DeveloperMode)
                            {
                                ...
                                string hash = Security2.Integrity(Process.GetCurrentProcess().MainModule.FileName);
                            ...
                            else
                            {
                                ...
                                {
                                    if (ApplicationSettings.Hash != Security2.Integrity(Process.GetCurrentProcess().MainModule.FileName))
                                    {
        ...
        public static bool AIOLogin(string AIO)
        {
            ...
            using (WebClient wc = new WebClient())
            {
                try
                {
                    ...
                    if (response[0] != Constants.Token)
                    {
                        MessageBox.Show("Security2 error has been triggered!", OnProgramStart.Name, MessageBoxButton.OK, MessageBoxImage.Error);
                        Process.GetCurrentProcess().Kill();
                    }
                    if (Security2.MaliciousCheck(response[1]))
                    {
    ...
    internal class Security2
    {
        ...
        public static void End()
        {
            if (!Constants.Started)
            {
                MessageBox.Show("No session has been started, closing for Security2 reasons!", OnProgramStart.Name, MessageBoxButton.OK, MessageBoxImage.Warning);
                Process.GetCurrentProcess().Kill();
            }
            else
            {
    ...
    internal class Encryption
    {
        ...
        public static string EncryptService(string value)
        {
            ...
            string final = encrypted + Security2.Obfuscate(property);
            return final;
        }
